...
class SpotServiceTest {
    ...
    @Nested
    @DisplayName("즐길거리 추가 조회(폴링) 테스트")
    class SpotPollingTest {

        @Test
        @DisplayName("즐길거리 추가 조회 코스 성공 - 초기화 완료")
        void getSpotsByStatus_completed_returnsSpotsList() {
            // given
            Long courseId = 1L;
            Course course = createMockCourse(courseId, COMPLETED);

            Spot spot1 = createMockSpot(101L, "Spot1", "Description1", null);
            Spot spot2 = createMockSpot(102L, "Spot2", "Description2", null);
            List<SpotInfoDto> mockSpots = List.of(SpotInfoDto.from(spot1), SpotInfoDto.from(spot2));

            when(courseRepository.findById(courseId)).thenReturn(Optional.of(course));
            when(spotRepository.findRandom3ByCourseId(courseId)).thenReturn(mockSpots);

            // when
            SpotDetailDto result = spotService.getSpotsByStatus(courseId);

            // then
            assertThat(result.courseId()).isEqualTo(courseId);
            assertThat(result.spotCount()).isEqualTo(2);
            assertThat(result.spotStatus()).isEqualTo(COMPLETED.name());
            assertThat(result.spots()).isEqualTo(mockSpots);
        ...
        @Test
        @DisplayName("즐길거리 추가 조회 코스 성공 - 초기화 진행 중")
        void getSpotsByStatus_notCompleted_returnsEmptyList() {
            // given
            Long courseId = 1L;
            Course course = createMockCourse(courseId, IN_PROGRESS);
            ...
            assertThat(result.courseId()).isEqualTo(courseId);
            assertThat(result.spotCount()).isEqualTo(0);
            assertThat(result.spotStatus()).isEqualTo(IN_PROGRESS.name()); // 상태값이 IN_PROGRESS인지 확인
            assertThat(result.spots()).isEmpty(); // 빈 리스트인지 확인

            verify(courseRepository).findById(courseId);
            verify(spotRepository, never()).findRandom3ByCourseId(anyLong());
        }

        @Test
        @DisplayName("즐길거리 추가 조회 실패 - 존재하지 않는 코스")
        void getSpotsByStatus_CourseNotFound_ThrowsException() {
            // given
            Long nonExistCourseId = 999L;

            when(courseRepository.findById(nonExistCourseId)).thenReturn(Optional.empty());

            // when, then
            // when, then
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> spotService.getSpotsByStatus(nonExistCourseId));
            assertThat(exception.getResponseCode()).isEqualTo(COURSE_NOT_FOUND);
